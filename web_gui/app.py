...
# Phase 9: white-label "About" page (client-requested 2026-08-19). Same
# generic-.env pattern as BRAND_NAME above -- CLIENT_NAME/CLIENT_ABOUT are
# plain .env values, editable via the existing Setup Wizard with no env_ops.py
# change needed, so each resold client's own business intro can show here
# without touching code per-client.
@app.get("/about", response_class=HTMLResponse)
def about_page(request: Request, _login=Depends(require_login), _auth=Depends(require_auth)):
    return render(
        request,
        "about.html",
        {
            "client_name": os.environ.get("CLIENT_NAME", "").strip(),
            "client_about": os.environ.get("CLIENT_ABOUT", "").strip(),
        },
    )
